...
class ExceedenceTrader:
    ...
    def save_tokens_to_aws(self, tokens):
    ...
    def save_tokens(self, tokens):
        ...
        if USE_AWS_SECRETS and secrets_client:
            success = self.save_tokens_to_aws(tokens)
            if success:
                return
            else:
    ...
    def load_tokens(self):
        ...
        if USE_AWS_SECRETS and secrets_client:
            tokens = self.load_tokens_from_aws()
            if tokens:
                return tokens
            else:
        ...
        # Fallback to local file storage
        if TOKEN_FILE:
            return self.load_tokens_from_file()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--strategy', type=str, choices=['reversal', 'momentum'], default='momentum',
                       help='Trading strategy: reversal (short at top, long at bottom) or momentum (long at top, short at bottom)')
    parser.add_argument('--auto', type=str, choices=['yes', 'no'], default='yes',
    ...
    parser.add_argument('--direction', type=str, choices=['long', 'short', 'both'], default='long',
                      help='Trading direction (long/short/both). Default is long')
